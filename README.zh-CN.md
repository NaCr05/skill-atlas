...
<table>
  <tr>
    <td width="50%" valign="top">
      <img src="artifacts/detail-desktop.png" alt="包含就绪状态与调用指引的 Skill 详情页"><br>
      <sub><b>检查 Skill</b> — 核对来源、就绪状态、依赖与调用指引。</sub>
    </td>
    <td width="50%" valign="top">
      <img src="artifacts/marketplace-desktop.png" alt="市场发现与安装前审查界面"><br>
      <sub><b>安全发现</b> — 比较市场候选，并在安装前审查完整 Skill 文件树。</sub>
    </td>
  </tr>
  <tr>
    <td colspan="2" align="center">
      <img src="artifacts/dashboard-mobile.png" alt="Skill Atlas 移动端目录" width="320"><br>
      <sub><b>随时使用</b> — 目录与调用流程会适配手机尺寸。</sub>
    </td>
  </tr>
</table>
...
本地清单、任务推荐和默认 Prompt 功能都不需要 API Key。AI 增强可以直接在**环境设置 → AI 连接配置台**中完成：选择提供商、填写模型和 API Key 后保存即可立即生效，刷新和重启后仍然保留。密钥使用 Windows 当前用户 DPAPI 加密，保存后不会再次回传到页面。保存失败时，界面会区分输入无效、Windows 加密失败、目录权限或文件占用等原因，并且不会把密钥写入日志或错误信息。
