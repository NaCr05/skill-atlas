...
import { inspectPermanentDeletion } from "@/core/lifecycle/skill-trash";
...
export async function POST(request: Request) {
  try {
    ...
    const review = await inspectPermanentDeletion(input);
    return Response.json(review, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    return apiErrorResponse(request, error, "PURGE_INSPECTION_FAILED");
  }
}
