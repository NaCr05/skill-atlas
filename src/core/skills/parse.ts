...
export function parseSkillDocument(
  ...
  const explicitDependencies = [
    ...stringArray(dependenciesConfig.skills),
    ...stringArray(metadata.related_skills),
  ];
  const mentionedDependencies = Array.from(
    body.matchAll(/\$([a-z][a-z0-9-]{1,63})\b/g),
    (match) => match[1],
  );
  const tags = [
  ...
  return {
    ...
    displayName:
      ...
      rawName ||
      folderName,
    description: rawDescription || "该 Skill 没有提供描述。",
    author:
    ...
    dependencies: Array.from(
      new Set([...explicitDependencies, ...mentionedDependencies]),
    ).filter((dependency) => dependency !== rawName),
    requiredTools,
