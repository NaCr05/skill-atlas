...
>
> 版本基线：`v0.2.0` 已把生命周期管理、目录工作台、响应式导航、大清单分页和 Windows 安装包统一到同一发布快照。
...
从 `v0.2.0` 起，版本号、源码标签、GitHub Release 与 Windows 安装包使用同一版本。判断普通用户能否获得某项后续功能时，仍应同时检查版本号、标签、Release 和安装包附件，而不是只看 `main`。
